...
int main()
{
    // Test 1: just return the points if it has only one point
    {
        std::vector<Point> points = { {1,2} };
        std::vector<Point> result = simplify<Point, float>(points);
        assert(result.size()==1 && result[0].x==1 && result[0].y==2);
    }
    
    
    // Test 2: just return the points if it has no points
    {
        std::vector<Point> points = { };
        std::vector<Point> result = simplify<Point, float>(points);
   ...
   // Test 3: simplifies points correctly with the given tolerance
    std::vector<Point> result = simplify<Point, float>(points, 5);
    assert(result.size() == simplified.size());
    for (size_t i=0; i<result.size(); i++) {
        if(!(result[i].x == simplified[i].x && result[i].y == simplified[i].y)) {
            std::cout << i << ": " << result[i].x << ", " << simplified[i].x << ", " << result[i].y << ", " << simplified[i].y << std::endl;
        }
    }
